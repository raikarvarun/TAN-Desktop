...
namespace TAN.Views
{
    ...
    public partial class ExpenseItemView : UserControl
    {
        private static List<ExpenseItemModel> _expenseItems;
        ...
        public ObservableCollection<ExpenseItemModel> ExpenseCatMainData
        {
        ...
        public ExpenseItemView(IEventAggregator events , IAPIHelper aPIHelper)
        {
            ...
            EXPENSEITEMDATAGRID.ItemsSource = ExpenseCatMainData;
            assginParties();
            EXPENSEITEMDATAGRID.SelectedIndex = 0;
        ...
        {
            ExpenseItemModel data = (ExpenseItemModel)EXPENSEITEMDATAGRID.SelectedItem;
            if (data != null)
            {
                if (data.ExpenseItemName != null)
                {
        ...
        private Task LoadDataAsync()
        {
            return Task.Factory.StartNew(() =>
            {
                ...
                _expenseItems = ExpenseItemSqllite.readAll();
                foreach (ExpenseItemModel c in _expenseItems)
                {
        ...
        private Task SearchDataAsync(string key)
        {
            return Task.Factory.StartNew(() =>
            {
                ...
                foreach (ExpenseItemModel c in _expenseItems.Where(s => s.ExpenseItemName.ToLower().Contains(key)).ToList())
                {
                    _expenseItemMaindata.Add(c);
                }
            });
